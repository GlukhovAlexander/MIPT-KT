...
//futimens (dst_fd, const struct timespec {sb.st_atim, sb.st_mtim})
...
int copy_file (int file_to_copy, int copy_of_the_file)
{
        ...
        ssize_t bytes_read;
        int result = 0;
        //Считываем информацию, после чего функция read возвращает количество прочитанных байтов
        while ((bytes_read = read(file_to_copy, read_inf, BUF_SIZE)) > 0)
        {
                //записываем информацию, которую считали
                //в writeall передаём куда писать, что и сколько байт
                if (writeall(copy_of_the_file, read_inf, (size_t)bytes_read) < 0)
                {
                        perror("File writing error"); //проверка ошибки записи
                        result = 8;
                        break;
                }
        }
        //Проверка ошибки при считывании
...
int copy_metadata(int file_to_copy, int copy_of_the_file) {
	...
	//Если успех, то fstat возвращает 0
	//fstat идентичен stat, но опрашиваемый файл задаётся в виде фалового дескриптора
	if (fstat(file_to_copy, &sb_to_copy) == 0) {
		//fchmod меняет права доступа к файлу, на который указывает открытый файловый дескриптор
		...
		//futimens обновляет времнные метки с точностью до наносекунд
...
int main (int argc, char const *argv[]) {
        //Ошибка, если неверное количество аргументов
        if (argc != 3) {
	...
	//Проверка на ошибку при закрытии
