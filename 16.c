...
int main(int argc, char* argv[]) {
    ...
    // Создаём очередь
    mqd_t queue = mq_open(argv[1], O_RDWR | O_CREAT, 0600, NULL); //создает новую очередь сообщений POSIX или открывает существующую очередь,
    ...
    if (mq_getattr(queue, &q_inf) == -1) { 
        perror("mq_getattr"); //В случае ошибки mq_getattr возвращает -1
    }

    PrintInfo(argv[1], &q_inf);

    // Вывод сообщения
    if (mq_send(queue, "hello", strlen("hello"), 0) == -1) { //добавляет сообщение, на которое указывает msg_ptr, в очередь сообщений, на которую ссылается дескриптор mqdes
        perror("mq_send");
    } 
    ...
    if (mq_receive(queue, buf, q_inf.mq_msgsize, NULL) == -1) { //удаляет самое старое сообщение с наивысшим приоритетом из очереди сообщений, на которую ссылается дескриптор mqdes, и помещает его в буфер, на который указывает msg_ptr
        perror("mq_receive");
        free(buf);
        return -1;
    }
